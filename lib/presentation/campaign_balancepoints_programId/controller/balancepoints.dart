...
class CampaignBalancePointsController1 extends GetxController {
  var searchQuery = ''.obs;
    ...
    try {
      ...
      if (result.isNotEmpty) {
        var earnedTransactions = result
            .where((transaction) => transaction.transactionType == 'balanced')
            .toList();
  ...
  void transactionhistoryAll() async {
    isLoading.value = true;
    try {
      var result = await CampaignCalculatedPointsService().transactiondataAll();
      if (result.isNotEmpty) {
        var earnedTransactions = result
            .where((transaction) => transaction.transactionType == 'balanced')
            .toList();
