class ClaimDetails {
  ...
  final String campaignSkuId; // Added field
  ...
  factory ClaimDetails.fromJson(Map<String, dynamic> json) {
    return ClaimDetails(
      ...
      quantity: json['quantity']?.toString() ?? '', // Handle null safely
      totalPoints: json['total_points'].toString(),
      invoiceImage: json['invoice_image'].toString(),
      campaignSkuId: json['campaign_sku_id']?.toString() ?? '', // Handle null safely
      ...
      multipleFiles: List<String>.from(json['Multiple Files'].map((x) => x.toString())),
      slabDetails: List<Map<String, String>>.from(
